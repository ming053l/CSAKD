...
def trainer(args):
    ...
    # args.network_mode = 1 # 1 : LRHSI+HRMSI
    ...
    model_t = DCSN_Teacher(args).to(args.device)
    ...
    if args.resume_ind>0 or os.path.isfile(args.resume_ckpt):
        if not os.path.isfile(args.resume_ckpt):
            args.resume_ckpt = os.path.join('pretrain_student', args.prefix, 'best.pth')
            # args.resume_ckpt = os.path.join('pretrain_student', args.prefix, 'last.pth')
        if not os.path.isfile(args.resume_ckpt):
            print(f"checkpoint is not found at {args.resume_ckpt}")
            raise 
        state_dict = torch.load(args.resume_ckpt)  
    ...
    # if args.lr_scheduler=='cosine':
    #     scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer_s, args.epochs-args.resume_ind, eta_min=1e-10, last_epoch=-1, verbose=False)
    # else:
    scheduler = optim.lr_scheduler.StepLR(optimizer_s, step_size=args.step_size, gamma=0.9) # 0.0001*0.9^11 = 0.000031(epoch = 30*11=330)
        # scheduler = optim.lr_scheduler.ExponentialLR(optimizer_s, gamma=0.9)
    ...
    for epoch in range(resume_ind, args.epochs): 
        ...
        for batch_idx, (X) in enumerate(train_loader):
            ...
            if args.network_mode == 2:
                # x  = cokey , x2 = lrhsi , x3 = hrmsi , y  = GT
                co_decoded = model_t(x, LRHSI=x2, HRMSI=x3)
            elif args.network_mode == 1:
                # x2 = lrhsi , x3 = hrmsi , y  = GT
                co_decoded = model_t(None, LRHSI=x2, HRMSI=x3)
            elif args.network_mode == 0:
                # x  = co , y  = GT
                co_decoded = model_t(x, LRHSI=None, HRMSI=None)
            ...
            while torch.isnan(loss2_t) and scheduler_t.get_last_lr()[0]>1e-12:
                print('Force learning rate decay to', scheduler_t.get_last_lr()[0]/5)
                
                args.resume_ckpt = os.path.join('teacher_checkpoint', args.prefix, 'last.pth')
                state_dict = torch.load(args.resume_ckpt)  
                model_t.load_state_dict(state_dict)
                
                optimizer_t = optim.Adam(model_t.parameters(), lr=scheduler_t.get_last_lr()[0]/5)  
                scheduler_t = optim.lr_scheduler.StepLR(optimizer_t, step_size=args.step_size)
                args.joint_loss = 0
            ...
            while torch.isnan(loss2_s) and scheduler.get_last_lr()[0]>1e-12:
                ...
                optimizer_s = optim.Adam(model.parameters(), lr=scheduler.get_last_lr()[0]/5)  
                scheduler = optim.lr_scheduler.StepLR(optimizer_s, step_size=args.step_size)
                args.joint_loss = 0
        ...
        if epoch% args.eval_step ==0:
            ...
            with torch.no_grad():
                ...
                for ind2, X in enumerate(val_loader):
                    ...
                    if args.network_mode==2:
                        val_dec = model_t(vx, LRHSI=vx2, HRMSI=vx3, mode=1)
                    elif args.network_mode==1:
                        val_dec = model_t(None, LRHSI=vx2, HRMSI=vx3, mode=1)
                    elif args.network_mode==0:
                        val_dec = model_t(vx, LRHSI=None, HRMSI=None, mode=1)
                    ...
                    start_time = time.time()
                    if args.network_mode==2:
                        val_dec_s = model(vx, LRHSI=vx2, HRMSI=vx3, mode=1)
                    elif args.network_mode==1:
                        val_dec_s = model(None, LRHSI=vx2, HRMSI=vx3, mode=1)
                    elif args.network_mode==0:
                        val_dec_s = model(vx, LRHSI=None, HRMSI=None, mode=1)
                ...
                print(' ')
                #print('[epoch: %d] Loss_T: %.3f, Loss-SAM_T: %.3f, Loss-BWS_T: %.3f, val-rmse: %.3f, val-SAM: %.3f, val-PSNR: %.3f, val-ERGAS: %.3f, lr: %f' % (epoch, 100*np.mean(running_loss_T), np.mean(running_sam_T), np.mean(running_bws_T), np.mean(rmses), np.mean(sams), np.mean(psnrs), np.mean(ergas),scheduler.get_last_lr()[0]))
